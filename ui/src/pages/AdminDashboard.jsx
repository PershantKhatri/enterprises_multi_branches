...
export default function AdminDashboard() {
  ...
  return (
    <div className="p-3 sm:p-6 lg:p-8 space-y-5 sm:space-y-8 bg-[#090d16] min-h-screen text-slate-100 select-none overflow-x-hidden">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-amber-500/10 via-slate-900 to-[#05070b] p-4 sm:p-6 rounded-2xl border border-amber-500/20 shadow-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            <div className="p-2 bg-amber-500/10 border border-amber-500/20 rounded-xl shrink-0">
              <Building2 className="w-5 h-5 text-amber-400" />
            </div>
            <h1 className="text-base sm:text-xl font-bold text-white tracking-tight leading-snug">Platform Owner Command Center</h1>
          </div>
                ...
                <div key={tenant._id} className="p-4 space-y-3 hover:bg-slate-950/40 transition">
                  ...
                  <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-900">
                    {tenant.status !== 'APPROVED' && (
                      <button
                        onClick={() => handleStatusChange(tenant._id, 'APPROVED')}
                        className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1 px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-lg transition shadow-md shadow-emerald-600/20 cursor-pointer"
                      >
                    ...
                    {tenant.status !== 'SUSPENDED' && (
                      <button
                        onClick={() => handleStatusChange(tenant._id, 'SUSPENDED')}
                        className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1 px-3 py-2 bg-rose-600/80 hover:bg-rose-600 text-white text-xs font-bold rounded-lg transition shadow-md shadow-rose-600/20 cursor-pointer"
                      >
                        <Ban size={14} /> Suspend
                      </button>
                    )}
